...
class RiwayatController extends Controller
{
    ...
    public function data_simpus_ckg(Request $request)
    {
        ...
        try {
            ...
            ])->timeout(120)->post('http://119.2.50.170/db_lb1/api/login', $data_login);
                        ...
                        // if($dt['nik']=="3321050110940006"){
                            if (!empty($dt['hasil_pemeriksaan'])) {
                                ...
                                $data = Riwayat::updateOrCreate(
                                    [
                                        'id_user' => $id_user, // Kondisi pencarian (unik)
                                        'id_pasien' => $id_pasien,
                                        'tanggal_pemeriksaan' => $dt['tanggal']
                                    ],
                                    [
                                        ...
                                        'hasil_pemeriksaan_lainnya' => $hp_lainnya ? json_encode($hp_lainnya) : null,
                                        'kesimpulan_hasil_pemeriksaan' => $kesimpulan_hasil_pemeriksaan,
                                        'program_tindak_lanjut' => $program_tindak_lanjut ? json_encode($program_tindak_lanjut) : null,
                                    ]
                                );
